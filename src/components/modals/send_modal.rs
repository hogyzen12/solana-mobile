...
// MWA approval overlay component shown during MWA transaction signing
#[cfg(target_os = "android")]
#[component]
fn MwaApprovalOverlay(oncancel: EventHandler<()>) -> Element {
    rsx! {
        div {
            class: "hardware-approval-overlay",
            div {
                ...
                h3 {
                    class: "hardware-approval-title",
                    "Confirm on Mobile Wallet"
                }
                p {
                    class: "hardware-approval-text",
                    "Please check your Seed Vault wallet and confirm the transaction."
                }
                button {
                    class: "hardware-cancel-button",
                    onclick: move |_| oncancel.call(()),
...
pub fn SendModalWithHardware(
    ...
    #[cfg(target_os = "android")]
    let mut show_mwa_approval = use_signal(|| false);

    #[cfg(target_os = "android")]
    let mwa_wallet_state = use_context::<Signal<WalletState>>();

    let show_mwa_approval_active = {
        #[cfg(target_os = "android")]
        {
            show_mwa_approval()
        }
    ...
    let mwa_overlay: Option<Element> = {
        #[cfg(target_os = "android")]
        {
            if show_mwa_approval() {
                Some(rsx! {
                    MwaApprovalOverlay {
                        oncancel: move |_| {
    ...
    let info_message: Option<Element> = {
        #[cfg(target_os = "android")]
        {
            if let WalletState::Pubkey(_) = mwa_wallet_state() {
                Some(rsx! {
                    div {
                        class: "info-message",
                        "Your Seed Vault will prompt you to approve the transaction"
                    }
                })
            } else if hardware_wallet.is_some() {
                Some(rsx! {
    ...
    let display_address = {
        #[cfg(target_os = "android")]
        {
            if let WalletState::Pubkey(pubkey) = mwa_wallet_state() {
                pubkey.to_string()
            } else if let Some(hw) = &hardware_wallet {
            ...
            div {
                ...
                div { class: "modal-buttons",
                    button {
                        ...
                        if sending() && !show_hardware_approval() && !show_mwa_approval_active { "Sending..." } else { "Send" }
                    }
                }
            }
        }
